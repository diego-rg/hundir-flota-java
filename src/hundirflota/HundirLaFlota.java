...
public class HundirLaFlota {
	...
	public static void main(String[] args) {
		...
		while (!juegoTerminado) {
			...
			//Sabemos si la tirada ha sido AGUA O TOCADO si el n�mero de puntos se ha decrementado.
			...
			//Si no ha ganado el jugador, le toca a la m�quina
			if (!juegoTerminado) {

				System.out.printf("PUNTOS RESTANTES DEL ORDENADOR: %d\n\n", puntosOrdenador);
				System.out.println("TURNO DEL ORDENADOR");
				tiroCorrecto = false;
				//Seguimos los mismos par�metros de comprobaci�n que en la tirada del usuario
				while (!tiroCorrecto) {
	...
	/*
	 * M�todo que sirve para que el ordenador pueda hacer un disparo
	 */
	...
	/*
	 * Metodo que aglutina la inicializaci�n de ambos mapas
	 */
	...
	/*
	 * M�todo que inicializa el mapa que mostramos al usuario
	...
	/*
	 * M�todo que inicializa un mapa de juego, colocando
	...
	private static void inicializaMapa(char[][] mapa) {
		...
		// Posible direcci�n de colocaci�n del barco
		char[] direccion = { 'V', 'H' };
		...
		for (int b : barcos) {
			...
			// Vamos de mayor tama�o a menor, para que sea menos
			// dificultoso encontrar un hueco
			boolean colocado = false;
			while (!colocado) {

				//Obtenemos una posici�n y direcci�n aleatorias
				int fila = aleatorio();
	...
	private static int[] pedirCasilla() {
		...
		//Pasamos la cadena a may�sculas
		linea = linea.toUpperCase();
		int[] t;


		// Comprobamos que lo introducido por el usaurio es correcto mediante una expresi�n regular
